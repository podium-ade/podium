// Package playbooks holds the profile Podium's own bot runs: profile.yaml, the playbooks
...
func TestTheBotsOwnProfileLoads(t *testing.T) {
	p, err := profiles.Load(".")
	require.NoError(t, err, "playbooks/ does not load; the real deployment would not start")
