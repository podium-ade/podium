...
export const tools = [
    {
        name: "delegate",
        description: "Run work in a Podium task, on a machine with a repository checked out and, depending " +
    ...
    },
    {
        ...
        description: "Where a delegated task has got to, and its answer once it has one. status is " +
    ...
    },
    {
        ...
        description: "Stop a delegated task. Use it when the work is no longer wanted or is plainly stuck — " +
...
export function newHandler(client) {
    ...
    try {
        switch (name) {
            case "delegate": {
                ...
                return ok(`Delegated to \`${delegation.playbook}\`.\n` +
...
export async function serve(input, write, handler, onWarn = () => { }) {
...
async function main() {
    ...
    if (url === "" || token === "") {
        process.stderr.write(`podium-mcp: needs ${UrlFlag} and ${TokenEnv}; this server is started by the agent ` +
            "runtime and is not meant to be run by hand\n");
        return 2;
    }
    await serve(process.stdin, (line) => process.stdout.write(line + "\n"), newHandler(new Client(url, token)), (message) => process.stderr.write(`podium-mcp: ${message}\n`));
    return 0;
}
