// Package local is the single-machine transport: no Tailscale, no mTLS, one shared bearer token. It
// refuses to bind anything but a loopback address, because the token is the only thing between
// a caller and the whole API. PODIUM_LOCAL_ALLOW_UNSAFE_LISTEN waives that for a container,
...
// Options configures the local transport.
type Options struct {
	...
	// host, so the operator has to say which it is. It is PODIUM_LOCAL_ALLOW_UNSAFE_LISTEN
...
const UnsafeListenVar = "PODIUM_LOCAL_ALLOW_UNSAFE_LISTEN"

// CheckListen reports whether addr can be served. The host must be unambiguously loopback,
// because that token is the only credential there is and a routable address publishes
...
func CheckListen(addr string, allowNonLoopback bool) error {
	...
	if err != nil {
		return fmt.Errorf("local transport: %q is not a host:port address: %w", addr, err)
	}
	if port == "" {
		return fmt.Errorf("local transport: %q has no port", addr)
	}
	if allowNonLoopback {
		return nil
	}
	if host == "" {
		return fmt.Errorf("local transport: refusing to listen on %q: it binds every interface, "+
			"and the local transport must be loopback-only (use %s, or set %s=1 if this address is "+
	...
	ip := net.ParseIP(host)
	if ip == nil {
		return fmt.Errorf("local transport: refusing to listen on %q: %q is not a loopback address "+
			"or literal IP (use %s)", addr, host, DefaultListen)
	}
	if !ip.IsLoopback() {
		return fmt.Errorf("local transport: refusing to listen on %q: %s is not a loopback address, "+
			"and the local transport trusts a static token (use %s, or set %s=1 if this address is "+
...
func (l *Listener) Listen(ctx context.Context) (net.Listener, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("local transport: listen on %s: %w", l.addr, err)
	}
	return ln, nil
}
