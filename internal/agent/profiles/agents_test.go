...
func TestAgentForFallsBackPlaybookThenProfileThenDefault(t *testing.T) {
	p := &Profile{Agent: AgentGrok}
	assert.Equal(t, AgentClaude, p.AgentFor(Playbook{Agent: AgentClaude}), "the playbook's own wins")
	assert.Equal(t, AgentGrok, p.AgentFor(Playbook{}), "then the profile's")

	empty := &Profile{}
	assert.Equal(t, DefaultAgent, empty.AgentFor(Playbook{}),
...
func TestEffortForFallsBackToTheProfile(t *testing.T) {
	...
	assert.Equal(t, EffortHigh, p.EffortFor(Playbook{Effort: EffortHigh}))
	assert.Equal(t, EffortLow, p.EffortFor(Playbook{}))
	assert.Empty(t, (&Profile{}).EffortFor(Playbook{}), "unset means the model's own default")
}

// A playbook that switches backend must not silently inherit an effort its new model cannot
// take. The playbook on its own validates — it names no model — so only the profile, which
// knows the effective triple, can catch it.
func TestAProfileRefusesAnInheritedEffortItsPlaybooksModelCannotTake(t *testing.T) {
	p := &Profile{
		Name: "podium", DisplayName: "Podium", Model: "claude-opus-5",
		Effort: EffortMax, DefaultPlaybook: "coder",
		Playbooks: map[string]Playbook{"coder": {
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), `playbook "coder"`)
...
func TestResolveAppliesTheOverrideOverThePlaybookOverTheProfile(t *testing.T) {
	p := &Profile{Agent: AgentClaude, Model: "claude-opus-5", Effort: EffortHigh}
	playbook := Playbook{Model: "claude-sonnet-5"}

	t.Run("no override is the playbook then the profile", func(t *testing.T) {
		got := p.Resolve(playbook, Override{})
	...
	t.Run("a model alone brings its own backend", func(t *testing.T) {
		...
		got := p.Resolve(playbook, Override{Model: "grok-4.6"})
	...
	t.Run("an effort alone leaves the model alone", func(t *testing.T) {
		got := p.Resolve(playbook, Override{Effort: EffortLow})
		assert.Equal(t, Choice{AgentClaude, "claude-sonnet-5", EffortLow}, got)
	})

	t.Run("all three", func(t *testing.T) {
		got := p.Resolve(playbook, Override{Agent: AgentGrok, Model: "grok-4.5", Effort: EffortMedium})
...
func TestResolveDropsAnInheritedEffortTheNewModelCannotTake(t *testing.T) {
	...
	p.Effort = EffortHigh
	assert.Equal(t, EffortHigh, p.Resolve(Playbook{}, Override{Model: "grok-4.6"}).Effort)
}
...
func TestValidateOverrideRefusesALevelTheChosenModelRejects(t *testing.T) {
	...
	bad := p.Resolve(Playbook{}, Override{Model: "grok-4.6", Effort: EffortMax})
	err := ValidateOverride(bad)
	require.Error(t, err)
	assert.Contains(t, err.Error(), `effort "max" is not one grok-4.6 accepts`)

	assert.NoError(t, ValidateOverride(p.Resolve(Playbook{}, Override{Model: "grok-4.6", Effort: EffortXHigh})))
}
